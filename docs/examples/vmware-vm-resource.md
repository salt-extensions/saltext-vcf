...
```bash
salt -G 'tier:production' vmware_vm.power_state
salt -G 'app:web' vmware_vm.info
salt -G 'app:web' vmware_vm.snapshot_create name=before-deploy memory=false
salt -G 'app:postgres' vmware_vm.reset
...
```bash
salt -G 'region:west' vmware_vm.power_off
```
