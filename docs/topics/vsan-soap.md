...
vCenter REST has no vSAN surface in VCF 9.x. `saltext-vmware` uses the
...
| Module | Surface |
|---|---|
| `vmware_vsan_cluster` | Cluster-level reconfigure (dedup/compression, encryption, ESA vs OSA, fault tolerance) |
| `vmware_vsan_disk` | Disk group + disk membership |
| `vmware_vsan_fault_domain` | Fault domain definition + host assignment |
| `vmware_vsan_health` | Health summary, silent health checks |
...
```bash
salt-call vmware_vsan_health.query_cluster_health cluster=domain-c9
salt-call vmware_vsan_health.list_silent_checks cluster=domain-c9
salt-call vmware_vsan_disk.list_disk_groups cluster=domain-c9
salt-call vmware_vsan_fault_domain.list_ cluster=domain-c9
```

## ESA

VCF 9.x defaults to vSAN ESA. Read paths handle ESA transparently.
`vmware_vsan_cluster.reconfigure` requires ESA-compatible spec inputs;
...
- Token cache keyed by `(host, username)`. Call
  `saltext.vmware.utils.vsan.invalidate_session(opts)` to refresh.
