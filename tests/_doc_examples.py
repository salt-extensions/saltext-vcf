...
Block shape we recognize::
    ...
    .. code-block:: bash
        ...
        salt '*' vcf_thing.func arg1 key=value
"""
...
import saltext.vcf.modules as _modules_pkg
...
class DocExample:
    ...
    Attributes:
        ...
        virtual_name: the module's ``__virtualname__`` (e.g. ``vcf_nsx_node``).
...
def collect_all_examples() -> list[DocExample]:
    """Walk every module in ``saltext.vcf.modules`` and collect examples."""
    out: list[DocExample] = []
    for info in pkgutil.iter_modules(_modules_pkg.__path__):
        if info.ispkg:
            continue
        try:
            mod = importlib.import_module(f"saltext.vcf.modules.{info.name}")
        except ImportError:
            continue
...
def resolve_callable(example: DocExample) -> Callable | None:
    ...
    try:
        mod = importlib.import_module(f"saltext.vcf.modules.{target_name}")
