"""Framework-interface tests for resources.vmware_vm."""

import pytest
import responses

from saltext.vmware.resources import vmware_vm as vm
...
def _inject(monkeypatch, resource_id, framework_opts):
    ...
    monkeypatch.setattr(
        ...
        "__context__",
        {
            KEY: {
                "initialized": True,
                "instances": tree["vmware_vm"]["instances"],
...
def test_grains_static_fields_only_on_unreachable_vcenter(
    ...
    g = vm.grains()
    assert g["resource_type"] == "vmware_vm"
...
def test_snapshot_create_delegates_to_soap(monkeypatch, framework_opts):
    ...
    monkeypatch.setattr("saltext.vmware.resources.vmware_vm.vim_vm_snapshot.create", stub)
...
def test_snapshot_list_delegates(monkeypatch, framework_opts):
    _inject(monkeypatch, "web-01", framework_opts)
    monkeypatch.setattr(
        "saltext.vmware.resources.vmware_vm.vim_vm_snapshot.list_",
