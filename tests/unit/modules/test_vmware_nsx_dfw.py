...
from saltext.vmware.modules import vmware_nsx_context_profile
from saltext.vmware.modules import vmware_nsx_firewall_rule
from saltext.vmware.modules import vmware_nsx_security_policy
from saltext.vmware.modules import vmware_nsx_service
...
def test_security_policy_list(mocked_responses):
    ...
    assert vmware_nsx_security_policy.list_() == {"results": []}
...
def test_firewall_rule_create(mocked_responses):
    ...
    vmware_nsx_firewall_rule.create("r", "sp", action="ALLOW")
...
def test_service_get(mocked_responses):
    ...
    assert vmware_nsx_service.get("HTTPS")["id"] == "HTTPS"
...
def test_context_profile_list(mocked_responses):
    ...
    assert vmware_nsx_context_profile.list_() == {"results": []}
