...
def test_already_matches(monkeypatch):
    monkeypatch.setattr(c, "get_shares", lambda o, cluster, profile=None: _shares())
    ret = st.shares("cluster-1", cpu={"shares_level": "normal"})
    assert ret["changes"] == {}
...
def test_cpu_drift_applied(monkeypatch):
    ...
    ret = st.shares("cluster-1", cpu={"shares_level": "high"})
...
def test_both_cpu_and_memory_drift(monkeypatch):
    ...
    ret = st.shares("cluster-1", cpu={"limit": 1000}, memory={"reservation": 512})
