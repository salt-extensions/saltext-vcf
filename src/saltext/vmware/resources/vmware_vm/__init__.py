"""
``vmware_vm`` resource type — one virtual machine per resource ID.

Each ``vmware_vm`` resource maps to a single VM, identified by its
...
CONTEXT_KEY = "vmware_vm_resource"
...
def _vcenter_cfg(resource_id):
    ...
    if not vcenter_id:
        raise ValueError(f"vmware_vm resource {resource_id!r} is missing the 'vcenter' reference")
    vcenters = _ctx().get("vcenter_instances", {})
    vc_cfg = vcenters.get(vcenter_id)
    if not vc_cfg:
        raise ValueError(
            f"vmware_vm resource {resource_id!r} references unknown vcenter "
...
def _opts():
    ...
    return {
        "pillar": {"saltext.vmware": {"vcenter": _vcenter_cfg(_resource_id())}},
    }
...
def init(opts):
    """Load vmware_vm and referenced vCenter instance configs into context."""
    tree = pillar_resources_tree(opts)
    vm_instances = tree.get("vmware_vm", {}).get("instances", {})
    ...
    log.debug(
        "vmware_vm resource init: managing %s VMs across vCenters %s",
        len(vm_instances),
...
def discover(opts):
    ...
    return list(pillar_resources_tree(opts).get("vmware_vm", {}).get("instances", {}))
...
def grains():
    ...
    try:
        ...
        log.warning("vmware_vm grains: vCenter lookup failed for %s: %s", rid, exc)
    return g
...
def ping():
    ...
    try:
        ...
        log.warning("vmware_vm ping: %s", exc)
        return False
    ...
    try:
        ...
        log.warning("vmware_vm ping failed for %s via %s: %s", _resource_id(), host, exc)
        return False
